...
function download() {
	...
	if ! [ -f $FILENAME ]; then
		downloader --no-check-certificate ${MIRROR}/source/$FILENAME.tar.gz
	fi

	if ! [ -f $FILENAME.sha1 ]; then
		downloader --no-check-certificate ${MIRROR}/source/$FILENAME.tar.gz.sha1
	fi
	if [ "$TYPE" == "vs" ] ; then
...
function build() {
	...
	if [ "$TYPE" == "osx" ] ; then
		...
		downloader http://wiki.openssl.org/images/7/70/Setenv-android.sh
