...
function download() {
	...
	if ! [ -f $FILENAME ]; then
		wget -nv --no-check-certificate --max-redirect=0 ${MIRROR}/source/$FILENAME.tar.gz
	fi

	if ! [ -f $FILENAME.sha1 ]; then
		wget -nv --no-check-certificate --max-redirect=0 ${MIRROR}/source/$FILENAME.tar.gz.sha1
	fi
	if [ "$TYPE" == "vs" ] ; then
