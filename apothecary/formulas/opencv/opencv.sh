...
function build() {
  ...
  # end if iOS
    ...
    emcmake cmake .. -DCMAKE_INSTALL_PREFIX="${BUILD_DIR}/${1}/build_$TYPE/install" \
      ...
      -DCPU_DISPATCH='' \
      -DCV_TRACE=OFF \
      -DCMAKE_C_FLAGS="-s USE_PTHREADS=1 -I/${EMSDK}/upstream/emscripten/system/lib/libcxxabi/include/ -msimd128" \
      -DCMAKE_CXX_FLAGS="-s USE_PTHREADS=1 -I/${EMSDK}/upstream/emscripten/system/lib/libcxxabi/include/ -msimd128" \
      -DBUILD_SHARED_LIBS=OFF \
      -DBUILD_DOCS=OFF \
      -DBUILD_EXAMPLES=OFF \
      -DBUILD_FAT_JAVA_LIB=OFF \
