...
function prepare() {
	if [ "$TYPE" == "osx" ] ; then
		...
		sed -i -e 's/productType = "com\.apple\.product-type\.library\.dynamic";/productType = "com\.apple\.product-type\.library\.static";/g'  Xcode/libusb.xcodeproj/project.pbxproj
	fi
}
